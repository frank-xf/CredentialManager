...
namespace bnb
{
    ...
    private:
        ...
        node_type* _Find(_FuncType _findFunc)
        {
        ...
        template<typename _FuncType>
        const node_type* _Find(_FuncType _findFunc) const
        {
            for (node_type* ptr = _first; ptr; ptr = ptr->_next)
                if (_findFunc(*ptr))
                    return ptr;
        ...
        template<typename _FuncType>
        bool _Update(_FuncType _findFunc, const node_item& item)
        {
        ...
        template<typename _FuncType>
        bool _Remove(_FuncType _findFunc)
        {
            for (node_type* ptr = _first; ptr; ptr = ptr->_next)
            {
                if (_findFunc(*ptr))
        ...
        template<typename _FuncType>
        bool _Move(_FuncType _findFunc, int offset)
        {
            if (0 < offset)
            {
    ...
    public:
        ...
        template<typename _FuncType>
        size_type RemoveIf(_FuncType _findFunc)
        {
            size_type nResult = 0;
